...
public class TourPicker {
	...
	public static Map<Track, Cycle> choose(Tour tour) {
		...
		Map<Track, Cycle> output = new HashMap<Track, Cycle>();
		...
		//iterates over tracks and checks if any of the cycles are too expensive to make a profit on the track
		//then iterates over positions in the track using MTCS to try and find the best cycle
		while(iterT.hasNext()) {
			currentTrack = (Track) iterT.next();
			Iterator iterC = cycleList.iterator();
			
			ArrayList<Player> players = new ArrayList<Player>();
			Map<String, GridCell> startingPositions = currentTrack.getStartingPositions();
			double[][] distractorMatrix = Consultant.buildDistractorMatrix(currentTrack);
			...
			while(iterC.hasNext()) {
				//if the track has a certain proportion or higher of obstacles or distractors then discount certain cycles
				...
				if(weightedTracks.get(currentTrack)[0] - currentCycle.getPrice() > 0) {
					//check the cycle from each potential starting position
					for (GridCell pos : startingPositions.values()) {
						SearchNode node = new SearchNode(pos, currentCycle, currentTrack,
						...
						} else if (node.getValue() - currentCycle.getPrice() > bestNode.getValue()) {
							bestNode = node;
							bestCycle = currentCycle;
						}
					}
			...
			//store tracks and their best cycle and expected winnings with that cycle
			trackResults.add(new EvaluatedTrack(currentTrack, bestCycle, bestNode.getValue()));
		}
		List<Track> trackCombo = new ArrayList<Track>();
		double maxPrize = 0;
		//check all combinations of tracks (1,2,3)
		for(EvaluatedTrack t : trackResults) {
			//if this track alone gives more than the current best set it to best
			...
			for(EvaluatedTrack x : trackResults) {
				//if this track is different to the previous and gives more when added set it and previous as best
				...
				if(t.getPrize() + x.getPrize() > maxPrize) {
					...
					if(x.getCycle().equals(t.getCycle())){
						maxPrize = t.getPrize() + x.getPrize() + x.getCycle().getPrice();
					} else {
				...
				for(EvaluatedTrack y : trackResults) {
					//if this track is different to the previous ones and gives more when added with the previous set them as best
					// check for repeat cycle usage
					if(y.equals(x)) {
						continue;
					}
					...
					if(t.getPrize() + x.getPrize() + y.getPrize() > maxPrize) {
						...
						if(x.getCycle().equals(y.getCycle())) {
							maxPrize = t.getPrize() + x.getPrize() + y.getPrize() + y.getCycle().getPrice();
						} else if (t.getCycle().equals(y.getCycle())) {
							maxPrize = t.getPrize() + x.getPrize() + y.getPrize() + y.getCycle().getPrice();
						} else {
							maxPrize = t.getPrize() + x.getPrize() + y.getPrize();
						}
		...
		//goes through the evaluated track list and selects the track and cycle from it that corresponds to the selected track
		for(EvaluatedTrack z : trackResults) {
			for(Track t : trackCombo) {
				if(z.getTrack().equals(t)) {
					output.put(z.getTrack(), z.getCycle());
				}
			}
